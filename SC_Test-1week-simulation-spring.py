...
if __name__ == "__main__":
    ...
    hourly_pv = [
        0, 0, 6.044277044, 6.471579694, 7.310273226, 8.505777887, 10.528426186, 13.393559196, 12.619480843,
        14.428565641, 15.327398694, 14.981701298, 13.954382182, 12.334800924, 9.428751112, 8.041240025, 7.000294295,
        6.356594767, 6.090010622, 0, 0, 0, 0
    ]
...
if __name__ == "__main__":
    ...
    # Hourly PV data spanning 168 hours
    hourly_load = [
        9.087224561, 7.70055481, 7.4631927, 7.363268636, 7.120727157, 7.252749642, 7.394540348, 9.301483008, 10.43706759, 11.69226893, 10.38862626, 8.730139787, 9.857443311, 12.06352184, 10.06311567, 11.28802692, 11.67713047, 12.93682358, 12.58062723, 10.33197728, 13.16570931, 14.26051688, 11.02816465, 8.393033846
    ]
...
if __name__ == "__main__":
    ...
    # Hourly PV data spanning 168 hours
    hourly_heat = [
        4, 24, 21, 26, 17, 22, 27, 22, 21, 19, 35, 9, 20, 11, 22, 11, 7, 17, 18, 20, 15, 5, 19, 11
    ]
...
while datetime.now() < end_time:
    ...
    # pv_sell_price = spot_price5min  # Spot market price for PV sell to grid (SEK/kWh)
    max_boiler_capacity = 3  # kW
    boiler_efficiency = 0.9  # Efficiency (assumed)
    P_batterymax = 7.2  # battery capacity


    # Define a function for grid upper bound
    def grid_upper_bound(model, t):
        return HSBelecLoad5min[t] - HSBpvOutput5min[t]  # Ensuring that grid import is minimized
    ...
    model.min_val = pyo.Var(model.T, within=pyo.NonNegativeReals, initialize=0)
    model.boiler_heat = pyo.Var(model.T, within=pyo.NonNegativeReals, bounds=(0, max_boiler_capacity))
    #model.P_extra = pyo.Var(model.T, within=pyo.NonNegativeReals, initialize=0)  # Extra load for thermal/battery/EV
    model.Ppv_use_limit = pyo.Var(model.T, within=pyo.NonNegativeReals)
    model.P_battery_limit = pyo.Var(model.T, within=pyo.NonNegativeReals)
    model.battery_charging = pyo.Var(model.T, within=pyo.Binary, initialize=0)
    # Battery state of charge (SOC) in kWh
    model.SOC_battery = pyo.Var(model.T, bounds=(0, P_batterymax), within=pyo.NonNegativeReals, initialize=0)


    # Objective Function to Maximize Self-Consumption
    ...
    # Ensure all PV output is used
    def rule_C2(model, t):
        return model.Ppv_use[t] + model.Pgrid_sell[t] + model.P_battery[t] <= HSBpvOutput5min[
            t]  # Ensure all PV is consumed directly


    model.C2 = pyo.Constraint(model.T, rule=rule_C2)


    # Ensure electrical balance (modified)
    def rule_C3(model, t):
        return model.pGrid[t] + model.Ppv_use[t] + model.P_battery[t] + model.Pgrid_sell[t] >= model.pHP[t] + float(
            HSBelecLoad5min[t])
    ...
    def rule_C4(model, t):
        return model.heatpeak >= model.hDH[t]  # Heat peak constraint
    ...
    def rule_C5(model, t):
        return model.elecpeak >= model.pGrid[t]  # Electrical peak constraint
    ...
    # def rule_C8(model, t):
    # PV usage (self-consumption + grid export) cannot exceed total PV production.
    # return model.Ppv_use[t] + model.Pgrid_sell[t] == HSBpvOutput5min[t]

    # Modify PV usage constraints
    def rule_C9(model, t):
        return model.Ppv_use[t] <= model.Ppv_use_limit[t] - 0.3 * model.P_battery[t]
    ...
    # def rule_C9(model, t):
    # Pgrid equals load demand minus PV production.
    # return model.pGrid[t] == HSBelecLoad5min[t] - HSBpvOutput5min[t]

    # battery only charges when PV output exceeds electrical load
    def rule_battery_charge(model, t):
        if t == 0:
            return model.SOC_battery[t] == 0  # Assume battery starts empty
        return model.SOC_battery[t] == model.SOC_battery[t - 1] + (HSBpvOutput5min.get(t, HSBpvOutput5min) - HSBelecLoad5min.get(t, HSBheatLoad5min)) * \
            model.battery_charging[t]


    model.battery_charge = pyo.Constraint(model.T, rule=rule_battery_charge)


    # battery discharges when PV output is less than the load
    def rule_battery_discharge(model, t):
        if t == 0:
            return model.SOC_battery[t] == 0  # Assume battery starts empty
        return model.SOC_battery[t] == model.SOC_battery[t - 1] - (
                HSBelecLoad5min[t] - HSBpvOutput5min[t]
        ) * (1 if pyo.value(model.P_battery[t]) < 0 else 0)

    model.battery_discharge = pyo.Constraint(model.T, rule=rule_battery_discharge)


    # Prevents SOC from exceeding battery capacity
    ...
    def save_optimization_results(model, output_file="optimization_resultsSPRING.csv"):
        """
